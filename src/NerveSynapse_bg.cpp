...
int NerveSynapse::calculate_delta(double error)
{
    ...
    {
        this->delta_weight = error;
        // this->outNode->node_delta = error;
    }
    else //隱藏層權重
    {
        //double hidden_delta = this->outNode->calc_node_error(this->outNode->node_delta);
        this->delta_weight += error * this->weight;
        // this->inNode->node_delta += delta_weight;
    }

    // cout << this->inNode->node_id << "-" << this->outNode->node_id << "\t delta=" << this->delta_weight << endl;

    this->inNode->notifyError(this->delta_weight);
    return 0;
}
...
void NerveSynapse::update_weight()
{
    ...
    double new_val = NEAT::learning_rate * this->inNode->getAxon() * this->delta_weight;

    this->weight += new_val; // update weights
